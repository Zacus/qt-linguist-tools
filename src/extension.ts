 ...
 * @LastEditors: zs
 * @LastEditTime: 2025-06-21 16:13:04
...
export function activate(context: vscode.ExtensionContext) {
  // 添加状态栏按钮
  const linguistButton = vscode.window.createStatusBarItem(
  ...
  // 更新状态栏菜单
  let showMenuCommand = vscode.commands.registerCommand(
    "qt-linguist.showMenu",
    async () => {
      const items = [
        {
      ...
      if (selected) {
        ...
        if (
          activeEditor &&
          selected.command !== "qt-linguist.compileAll" &&
