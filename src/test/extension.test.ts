...
suite('Qt Linguist Extension Test Suite', () => {
    ...
    suiteSetup(() => {
        ...
        // 创建测试用的 ts 文件
        fs.writeFileSync(testTsFile, '<?xml version="1.0" encoding="utf-8"?><TS version="2.1"></TS>');
        // 创建普通文本文件
    ...
    test('2. Qt Linguist 命令应该被正确注册', async () => {
        ...
        assert.ok(commands.includes('qt-linguist.openInLinguist'), '未找到 openInLinguist 命令');
    });

    test('3. 对非 .ts 文件应该拒绝打开', async () => {
        try {
            await vscode.commands.executeCommand('qt-linguist.openInLinguist', vscode.Uri.file(testTxtFile));
    ...
    test('4. 应该能正确打开 .ts 文件', async () => {
        try {
            await vscode.commands.executeCommand('qt-linguist.openInLinguist', vscode.Uri.file(testTsFile));
