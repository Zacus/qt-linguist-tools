...
export async function getQtPaths(): Promise<QtPaths> {
            ...
            for (const expandedPath of expandedPaths) {
                ...
                for (const checkPath of [binPath, directPath]) {
                    if (!linguistPath) {
                        ...
                        if (await fileUtils.fileExists(linguistCandidate) && await fileUtils.isExecutable(linguistCandidate)) {
                    ...
                    if (!lreleasePath) {
                        ...
                        if (await fileUtils.fileExists(lreleaseCandidate) && await fileUtils.isExecutable(lreleaseCandidate)) {
                    ...
                    if (!lupdatePath) {
                        ...
                        if (await fileUtils.fileExists(lupdateCandidate) && await fileUtils.isExecutable(lupdateCandidate)) {
    ...
    if (platform === 'darwin' && !linguistPath) {
        ...
        for (const appPath of macLinguistPaths) {
            if (await fileUtils.fileExists(appPath) && await fileUtils.isExecutable(appPath)) {
