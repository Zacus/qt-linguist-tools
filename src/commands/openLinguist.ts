...
const openLinguistCommand = vscode.commands.registerCommand(
    ...
    try {
      ...
      if (platform === "darwin") {
        if (qtPaths.linguist.includes(".app/")) {
          ...
          const appPath = qtPaths.linguist.replace(
      ...
      vscode.window.showInformationMessage(
        `Qt Linguist 打开成功: ${path.basename(filePath)}`
      );
    } catch (error) {
      ...
      vscode.window
        .showErrorMessage("Qt 路径可能不正确，请检查设置", setPath)
        .then((selection) => {
...
async function activateLinguistWindow(): Promise<void> {
  try {
    ...
    if (platform === "darwin") {
      // macOS: 激活linguist应用
      await fileUtils.execAsync(
        `osascript -e 'tell application "linguist" to activate' 2>/dev/null || osascript -e 'tell application "Qt Linguist" to activate' 2>/dev/null || true`
      );
    } else if (platform === "win32") {
...
async function checkIfLinguistIsOpen(filePath: string): Promise<boolean> {
  try {
    ...
    if (platform === "darwin") {
      // macOS: 检查进程参数
      const { stdout } = await fileUtils.execAsync(
        `ps -eo pid,args | grep linguist`
      );
      ...
      const { stdout } = await fileUtils.execAsync(
        `wmic process where "name='linguist.exe'" get ProcessId,CommandLine /format:csv`
      );
      return (
        stdout.includes(fileName) ||
        stdout.includes(fullPath.replace(/\\/g, "\\\\"))
      );
    } else {
      // Linux: 检查进程参数
      const { stdout } = await fileUtils.execAsync(
