...
class TranslationUpdater {
     ...
     */
    private parseLupdateOutput(stdout: string, stderr: string): { 
        warnings: string[], 
        errors: string[], 
        hasErrors: boolean 
    } {
    ...
    private async prepareUpdateTasks(files: vscode.Uri[]): Promise<UpdateTask[]> {
        ...
        for (const file of files) {
            try {
                ...
                if (stats.isFile() && file.fsPath.endsWith('.ts') && !file.fsPath.endsWith('.d.ts')) {
                    // 直接选中的是 .ts 文件
                    ...
                    for (const tsFile of tsFiles) {
                        tasks.push({
                            sourceDir,
                            tsFile,
                            displayName: `${path.basename(tsFile)} (from ${path.basename(file.fsPath)})`
                        });
                    }
                }
    ...
    private async updateSingleTask(
        ...
        try {
            ...
            const { stdout, stderr } = await this.executeLupdate(command, taskId, token);
            
            const { warnings, errors, hasErrors } = this.parseLupdateOutput(stdout, stderr);
    ...
    async update(
        ...
        try {
            ...
            for (let i = 0; i < tasks.length; i++) {
                ...
                progress.report({
                    increment: i === 0 ? progressPercent : (progressPercent - ((i - 1) / tasks.length) * 100),
                ...
                // 记录进度
                console.log(`任务 ${i + 1}/${tasks.length} 完成: ${result.success ? '成功' : '失败'}`);
            }
...
function showUpdateResults(results: UpdateResult[]): void {
    ...
    const warningResults = results.filter(r => r.warnings && r.warnings.length > 0);
...
const updateTranslationCommand = vscode.commands.registerCommand(
    ...
    async (uri: vscode.Uri, uris?: vscode.Uri[]) => {
        ...
        try {
            ...
            if (!qtPaths?.lupdate) {
                vscode.window.showErrorMessage('未找到 lupdate 工具，请检查 Qt 安装路径配置');
                return;
            }
            ...
            try {
                ...
                vscode.window.showErrorMessage(`lupdate 工具不存在: ${qtPaths.lupdate}`);
                return;
            }
            ...
            await vscode.window.withProgress({
                ...
                try {
                    ...
                    if (token.isCancellationRequested) {
                        vscode.window.showWarningMessage(`更新已取消 - 已处理: 成功 ${successful} 个，失败 ${failed} 个`);
                        return;
                    }
                    ...
                    const warningMessage = totalWarnings > 0 ? `，警告 ${totalWarnings} 个` : '';
