...
class QmFileGenerator {
     ...
     * 构建 lrelease 命令
     */
    private buildLreleaseCommand(tsFilePath: string, options?: {
        compress?: boolean;
        verbose?: boolean;
        outputDir?: string;
    }): string {
        ...
        if (options?.outputDir) {
            const outputFile = path.join(options.outputDir, 
    ...
    private async compileSingleFile(
        ...
        try {
            ...
            if (stderr) {
                ...
                if (!lowerStderr.includes('warning') && 
    ...
    private async compileFiles(
        ...
        for (let i = 0; i < files.length; i += this.BATCH_SIZE) {
            ...
            progress.report({
                ...
                message: `编译批次 ${Math.floor(i / this.BATCH_SIZE) + 1} (${i + 1}-${Math.min(i + this.BATCH_SIZE, files.length)}/${total})`
            });
...
function showCompileResults(results: CompileResult[]): void {
    ...
    if (successResults.length > 0) {
        ...
        content += `**总计:** ${successResults.length} 个文件，${formatSize(totalSize)}\n\n`;
...
const generateQmCommand = vscode.commands.registerCommand(
    ...
    async (uri: vscode.Uri, uris?: vscode.Uri[]) => {
        try {
            ...
            // 过滤有效的TS文件
            const { validFiles, invalidFiles } = await fileUtils.filterTsFiles(selectedFiles);
            ...
            if (invalidFiles.length > 0) {
                vscode.window.showWarningMessage(
                    `已跳过 ${invalidFiles.length} 个无效文件: ${invalidFiles.slice(0, 3).join(', ')}${invalidFiles.length > 3 ? '...' : ''}`
                );
            }
            ...
            await vscode.window.withProgress({
                ...
                if (failed > 0) {
                    ...
                    } else if (action === '打开输出目录' && results.some(r => r.success)) {
                        // 打开第一个成功编译文件的目录
