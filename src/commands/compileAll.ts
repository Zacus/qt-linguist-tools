 ...
 * @LastEditors: zs
 * @LastEditTime: 2025-06-21 16:12:28
...
function showFailedFiles(failedResults: CompileResult[]): void {
  const failedFiles = failedResults
    .map((r) => `• ${path.basename(r.file)}: ${r.error || "未知错误"}`)
    .join("\n");

  vscode.window.showErrorMessage(`编译失败的文件:\n${failedFiles}`, {
    modal: false,
  });
}
...
const compileAllCommand = vscode.commands.registerCommand(
  ...
  async () => {
    try {
      ...
      // 4. 确认编译
      const proceed = await vscode.window.showQuickPick(["是", "否"], {
