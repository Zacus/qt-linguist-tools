...
class TranslationUpdater {
     ...
     * 执行批量更新
     */
    async execute(strategy: UpdateStrategy, tsFiles: vscode.Uri[]): Promise<UpdateResult[]> {
        switch (strategy) {
        ...
        try {
            ...
            for (let i = 0; i < tasks.length; i += this.concurrencyConfig.batchSize) {
                if (this.token.isCancellationRequested) {
                    ...
                    const cancelledResults = remainingTasks.map(task => ({
                        success: false,
                        file: task.name,
                        message: '任务已取消'
                    } as UpdateResult));
                ...
                // 批次间短暂延迟，避免系统过载
                if (i + this.concurrencyConfig.batchSize < tasks.length && !this.token.isCancellationRequested) {
...
/**
   ...
   /**
    ...
    private async updateFromProFiles(): Promise<UpdateResult[]> {
        ...
        return this.executeConcurrentTasks(tasks, (task) => this.updateProFile(task.data));
    }

    /**
     * 按目录分组更新 - 并发版本
     */
    private async updateByDirectory(tsFiles: vscode.Uri[]): Promise<UpdateResult[]> {
        const dirGroups = this.groupFilesByDirectory(tsFiles);
        const tasks: TaskItem<[string, vscode.Uri[]]>[] = Array.from(dirGroups.entries()).map(([dir, files], index) => ({
            data: [dir, files],
            index,
            name: path.basename(dir)
        }));

        return this.executeConcurrentTasks(tasks, (task) => this.updateDirectoryEnhanced(task.data[0], task.data[1]));
    }

    /**
     * 逐个文件更新 - 并发版本
     */
    private async updateIndividualFiles(tsFiles: vscode.Uri[]): Promise<UpdateResult[]> {
        ...
        return this.executeConcurrentTasks(tasks, (task) => this.updateSingleFile(task.data));
    }
    ...
    private async processFiles<T>(
        ...
        console.warn('processFiles is deprecated, use executeConcurrentTasks instead');
        const results: UpdateResult[] = [];
     ...
     * 改进的目录更新 - 增加错误处理
    //  */
    private async updateDirectory(dir: string, files: vscode.Uri[]): Promise<UpdateResult> {
        ...
        return this.safeExecute(async () => {
            ...
            return {
                success: true,
                file: dirName,
                message: this.extractLastLine(stdout) || `更新完成 (${files.length}个文件)`
            };
        }, `更新目录 ${dirName}`);
    }


      private async updateDirectoryEnhanced(dir: string, files: vscode.Uri[]): Promise<EnhancedUpdateResult> {
        const dirName = path.basename(dir);
        const startTime = Date.now();
        ...
        return this.safeExecute(async () => {
            ...
            // 解析输出获取详细信息
            const parseResult = LupdateOutputParser.parseDirectoryOutput(stdout, stderr, files);
    ...
    // // 修复版本2 - 更简洁但可靠的版本
    private groupFilesByDirectory(tsFiles: vscode.Uri[]): Map<string, vscode.Uri[]> {
     ...
     * 改进的类型安全的项目名称获取
     */
    private getItemName(item: vscode.Uri | [string, vscode.Uri[]] | unknown): string {
...
class LupdateOutputParser {
     ...
     */
    static parseDirectoryOutput(stdout: string, stderr: string, inputFiles: vscode.Uri[]): {
        ...
        result.stats.warnings = warningLines.filter(line => line.trim().length > 0);
        ...
        inputFiles.forEach(file => {
            ...
            // 检查是否在输出中被提及
            const wasProcessed = mentionedFiles.has(fileName) || 
        ...
        if (result.processedFiles.length === 0 && result.failedFiles.length === 0 && !stderr.trim()) {
...
class ResultAnalyzer {
    ...
    get cancelledCount(): number {
        return this.results.filter(r => !r.success && r.message === '任务已取消').length;
    }
    ...
    get averageDuration(): number {
        const durationsResults = this.results.filter(r => r.success && r.duration);
        if (durationsResults.length === 0) return 0;
        
        return durationsResults.reduce((sum, r) => sum + (r.duration || 0), 0) / durationsResults.length;
    }

    getSummaryMessage(cancelled: boolean): string {
        const prefix = cancelled ? '批量更新已取消' : '批量更新完成';
        const cancelInfo = this.cancelledCount > 0 ? `，取消 ${this.cancelledCount} 个` : '';
    ...
    formatResults(): string {
        ...
        const performanceInfo = avgDuration > 0 
            ? `平均处理时间: ${avgDuration.toFixed(0)}ms\n总处理数量: ${this.results.length}\n成功率: ${((this.successCount / this.results.length) * 100).toFixed(1)}%\n\n`
            : `总处理数量: ${this.results.length}\n成功率: ${((this.successCount / this.results.length) * 100).toFixed(1)}%\n\n`;
...
class EnhancedResultAnalyzer {
    ...
    formatDetailedResults(): string {
        ...
        const summary = `总体统计:\n` +
                       ...
                       `- 成功率: ${totalFiles > 0 ? ((this.totalProcessedFiles / totalFiles) * 100).toFixed(1) : 0}%\n\n`;
        ...
        this.results.forEach((result, index) => {
            ...
            detailContent += `${status} 目录 ${index + 1}: ${result.file}${duration}\n`;
            ...
            if (result.directoryInfo) {
                ...
                if (info.processedFiles.length > 0) {
                    detailContent += `   成功 (${info.processedFiles.length}): ${info.processedFiles.join(', ')}\n`;
                }
                
                if (info.failedFiles.length > 0) {
                    detailContent += `   失败 (${info.failedFiles.length}): ${info.failedFiles.join(', ')}\n`;
                }
                
                if (info.skippedFiles.length > 0) {
                    detailContent += `   跳过 (${info.skippedFiles.length}): ${info.skippedFiles.join(', ')}\n`;
                }
            }
            ...
            if (result.stats) {
                ...
                if (stats.sourceStrings) statInfo.push(`源字符串: ${stats.sourceStrings}`);
                if (stats.newStrings) statInfo.push(`新增: ${stats.newStrings}`);
                if (stats.modifiedStrings) statInfo.push(`修改: ${stats.modifiedStrings}`);
    ...
    getSummaryMessage(): string {
        const totalFiles = this.totalProcessedFiles + this.totalFailedFiles;
        return `批量更新完成: ${this.results.length} 个目录, ` +
 ...
 * 获取用户选择的更新策略和并发配置
 */
async function getUserStrategyAndConfig(): Promise<{ strategy: UpdateStrategy; config?: Partial<ConcurrencyConfig> } | undefined> {
...
async function handleUpdateAllCommand(): Promise<void> {
    try {
        ...
        await vscode.window.withProgress({
            ...
            const startTime = Date.now();
            const updater = new TranslationUpdater(qtPaths, progress, token, config);
            ...
            // 分析并显示结果
            const analyzer = new ResultAnalyzer(results);
            const message = `${analyzer.getSummaryMessage(token.isCancellationRequested)} (耗时: ${(totalTime / 1000).toFixed(1)}s)`;
...
// 注册命令
const updateAllCommand = vscode.commands.registerCommand('qt-linguist.updateAll', handleUpdateAllCommand);
